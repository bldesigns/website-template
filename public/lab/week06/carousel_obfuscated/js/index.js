...
//everytime you click skip increment
//everytime you click back decrease
...
function skipImage(){
	if (imageNum!==maxNumImage){
		imageNum++;
	} else if (imageNum === maxNumImage) {
		alert ('end of images');
	}
}

function changeImage(){
	$('#image-to-vote-on').attr('src','images/food'+imageNum+'.jpg');
}

function changeImage2(){
	var imageNumber = 1;
	for (var i = 0; i<maxNumImage; i++){
		imageSources[i]=['images/food'+imageNumber+'.jpg'];
		imageNumber++;
	}
	//alert (imageSources.length);
}

function yangChangeImage(){
	alert('food'+imageNum+'.jpg');
	$('#image-to-vote-on').attr('src',images[imageNum]);
}